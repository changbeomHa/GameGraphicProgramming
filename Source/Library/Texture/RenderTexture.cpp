...
namespace library
{
	...
	HRESULT RenderTexture::Initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext) {
		...
		hr = pDevice->CreateTexture2D(&desc_texture, NULL, &m_texture2D);
		if (FAILED(hr))
			return hr;


		D3D11_RENDER_TARGET_VIEW_DESC desc_rendertargetview =
		{
		...
		hr = pDevice->CreateRenderTargetView(m_texture2D.Get(), &desc_rendertargetview, m_renderTargetView.GetAddressOf());
		if (FAILED(hr))
			return hr;


		D3D11_SHADER_RESOURCE_VIEW_DESC desc_shaderResourceView =
		{
			.Format = desc_texture.Format,
		...
		hr = pDevice->CreateShaderResourceView(m_texture2D.Get(), &desc_shaderResourceView, m_shaderResourceView.GetAddressOf()
		);
		if (FAILED(hr))
			return hr;
